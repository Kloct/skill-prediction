	...
	reload() {
		...
		if(this.mod.settings.debug.skills) {
			...
			for(let options of [
				...
				['C_PRESS_SKILL', 5, _ => _.press, _unkn2, _unkn3],
				['C_START_TARGETED_SKILL', 7, _ => `[${_.targets.map(t => `${gameId(t.gameId)} ${t.unk}`).join(',')}]`],
				['C_START_COMBO_INSTANT_SKILL', 6, _ => `[${_.targets.map(t => `${t.unk1} ${gameId(t.target)} ${t.unk2}`).join(',')}]`],
				['C_START_INSTANCE_SKILL', 8, _ => [_.continue, `[${_.targets.map(t => `${t.arrowId} ${gameId(t.target)} ${t.hitCylinderId}`).join(',')}]`]],
