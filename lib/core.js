...
module.exports = function SPCore(mod) {
	...
	function handleStartSkill(type, event, info, data) {
		...
		function runPassive(passive) {
			if(!passive) return
			let hasPassiveGroup = false
			if(passive.group) for (let group of passive.group) if(player.itemPassivityGroups.has(group)&&event.skill.id==60100) hasPassiveGroup = true;
			if(!currentSkillCategory.includes(passive.conditionCategory) && !hasPassiveGroup) return
